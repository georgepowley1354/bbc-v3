...
(function() {
  ...
  function _addPhoto(){ var fi=document.getElementById('ph-new-file'); _readFile(fi,async function(dataUrl){ var cap=_v('ph-new-cap'),cat=_v('ph-new-cat'); var arr=KMCData.get('gallery'); await KMCData.addItem('gallery',{src:dataUrl||('./krismidcity-images/gallery-'+(arr.length+1)+'.jpg'),alt:cap||'Gallery photo',caption:cap,category:cat,date:new Date().toISOString().slice(0,10)}); renderGallery(); _toast('Photo added!'); }); }
  function _saveTeam(id){ var fi=document.getElementById('tm-photo-'+id); _readFile(fi,async function(dataUrl){ var u={name:_v('tm-name-'+id),role:_v('tm-role-'+id),bio:_v('tm-bio-'+id)}; if(dataUrl)u.photo=dataUrl; await KMCData.updateItem('team',id,u); renderTeam(); _toast('Saved!'); }); }
  async function _delTeam(id){ if(!confirm('Remove?'))return; await KMCData.removeItem('team',id); renderTeam(); }
  function _addTeam(){ var n=_v('tm-new-name').trim(); if(!n)return alert('Enter a name.'); var fi=document.getElementById('tm-new-photo'); _readFile(fi,async function(dataUrl){ await KMCData.addItem('team',{name:n,role:_v('tm-new-role'),bio:_v('tm-new-bio'),photo:dataUrl||''}); renderTeam(); _toast('Added!'); }); }
